function SuperSurvivorPlayerInit(player)
    ...
    if (player:getPlayerNum() == 0) then
        ...
        if (player:getModData().LockNLoad == nil) and (SuperSurvivorGetOptionValue("LockNLoad") == true) then
            ...
            if (wife) then
                for i = 1, 8 do
                    wife:Get():LevelPerk(Perks.FromString("Aiming"))
                end
            end
...
local function SuperSurvivorPVPHandle(wielder, victim, weapon, damage)
    ...
    logFunction("SuperSurvivorPVPHandle")
		...
		logValue("wielder id",wielder:getModData().ID)
		logValue("victim id",	victim:getModData().ID)
		logValue("damage",	damage)
    ...
    if (shotPartshotPart ~= nil) and (SSV:getID() ~= 0) then
        ...
        logValue("extraDamage", tostring(extraDamage))
    ...
    if not instanceof(victim, "IsoPlayer") then
        log("victim is not an human")
        logFunction("SuperSurvivorPVPHandle")
        return
    end
    ...
    -- end
    if IsNpcDamageBroken and instanceof(wielder, "IsoPlayer") and
        not (victim:isLocalPlayer()) then
